...
public class HungerEffectBerry extends EffectBerry {

    public HungerEffectBerry(Settings settings) {
        super(settings);
    }

    @Override
    public ItemStack getPickStack(BlockView world, BlockPos pos, BlockState state) {
        return new ItemStack(ModItems.HUNGER_EFFECT_BERRY);
    }
        ...
        if (entity instanceof LivingEntity && entity.getType() != EntityType.FOX && entity.getType() != EntityType.BEE) {
            ...
            if (!world.isClient && (Integer)state.get(AGE) > 0 && (entity.lastRenderX != entity.getX() || entity.lastRenderZ != entity.getZ())) {
                ...
                if (d >= 0.003000000026077032 || e >= 0.003000000026077032) {
                    ((LivingEntity) entity).addStatusEffect(new StatusEffectInstance(StatusEffects.HUNGER, 80, 0, false, false, false));
                }
            }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (!bl && player.getStackInHand(hand).isOf(Items.BONE_MEAL)) {
            ...
            dropStack(world, pos, new ItemStack(ModItems.HUNGER_EFFECT_BERRY, j + (bl ? 1 : 0)));
